...
import pack.cluster as cluster
import pack.geometry as geometry
import pack.geo2rot as geo2rot
        ...
        for k in range(0, len(lat)):
            ...
            if h_veg_tmp > 0.:
                ...
                if data1_tmp >= thr_val:
                    ...
                    if h_veg_tmp >= 5 :
                        LAD_C[0,:,1,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,10)-5)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 10 :
                        LAD_C[0,:,2,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,15)-10)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 15 :
                        LAD_C[0,:,3,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,20)-15)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 20 :
                        LAD_C[0,:,4,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,25)-20)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 25 :
                        LAD_C[0,:,5,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,30)-25)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 30 :
                        LAD_C[0,:,6,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,35)-30)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 35 :
                        LAD_C[0,:,7,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,40)-35)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 40 :
                        LAD_C[0,:,8,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,45)-40)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 45 :
                        LAD_C[0,:,9,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,50)-45)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 50 :
                        LAD_C[0,:,10,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,55)-50)/h_ug * area_lidar / area_grid
                    if h_veg_tmp >= 55 :
                        LAD_C[0,:,11,lat_idx,lon_idx] += LAD_spec * (min(h_veg_tmp,60)-55)/h_ug * area_lidar / area_grid

                else:
                    # Out-canyon vegetation
                    LAI_URB[0,lat_idx,lon_idx] += LAD_spec * area_lidar * (h_veg_tmp-h_cbase) / area_grid
